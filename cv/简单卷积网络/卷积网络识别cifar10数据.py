    ...
    def conv2d_layer(self):
        ...
        with tf.variable_scope('conv2d', reuse=tf.AUTO_REUSE):
            for i in range(2):
                ...
                conv2d_max_pool = tf.layers.max_pooling2d(
                    conv2d_data, pool_size=[2, 2], strides=[2, 2])
            ...
            self.vari = tf.get_collection(
                tf.GraphKeys.TRAINABLE_VARIABLES, scope='conv2d')
    ...
    def loss(self):
        ...
        self.loss = tf.reduce_mean(
            tf.nn.sparse_softmax_cross_entropy_with_logits(
                labels=label_place, logits=self.dense3))
...
path = os.path.join(
    r'C:\\Users\\85256\OneDrive\学习资料\软件学习\深度学习\code\cv\data\cifar 10 data',
    'data_batch_1')
